...
abstract class DatabaseBackupRestoreService {
  ...
  Future _copyDataFromSourceToTemp(Database destinationDb) async {
    try {
      ...
      await destinationDb.database.delete(TableConstant.userTable);

      Batch batch = destinationDb.batch();

      // ! users - preserve original IDs
      final users = await ref.read(userProviderRepository).getAllUsers();
      for (var u in users) {
        batch.insert(
          TableConstant.userTable,
          u.toJsonWithoutForCloud(),
          conflictAlgorithm: ConflictAlgorithm.replace,
        );
      }

      // ! products
      final productsReponse = await ref
          .read(productProviderRepository)
          .fetchProductsForBackupToCloud();

      await productsReponse.fold<Future>((l) async {}, (r) async {
        for (var p in r) {
          batch.insert(TableConstant.productTable, p.toJsonWithoutId());
        }
      });
      // ! categories
      ...
      for (var item in categories) {
        batch.insert(TableConstant.categoryTable, item.toJson());
      }

      // ! restaurant stock
      final restaurantStock = await ref
          .read(restaurantProviderRepository)
          .fetchAllStockItems();

      await restaurantStock.fold<Future>((l) async {}, (r) async {
        for (var i in r) {
          batch.insert(TableConstant.restaurantStockTable, i.toMap());
        }
      });
      ...
      final ingredients = await ref
          .read(posDbProvider)
          .database
          .query(TableConstant.ingredientTable);
      for (var ingredient in ingredients) {
        batch.insert(TableConstant.ingredientTable, ingredient);
      }

      //! sandwich ingredients (all records)
      final sandwichIngredients = await ref
          .read(posDbProvider)
          .database
          .query(TableConstant.sandwichesIngredientTable);
      for (var sandwichIngredient in sandwichIngredients) {
        batch.insert(
          TableConstant.sandwichesIngredientTable,
          sandwichIngredient,
        );
      }

      //! sales ingredients (linked to receipts from this year)
      final currentYear = DateTime.now().year;
      final salesIngredients = await ref
          .read(posDbProvider)
          .database
          .rawQuery(
            '''
        SELECT si.* FROM ${TableConstant.salesIngredientTable} si
        INNER JOIN ${TableConstant.receiptTable} r ON si.receiptId = r.id
        WHERE CAST(SUBSTR(r.receiptDate, 1, 4) AS integer) = ?
      ''',
            [currentYear],
          );
      for (var salesIngredient in salesIngredients) {
        batch.insert(TableConstant.salesIngredientTable, salesIngredient);
      }

      //! receipts
      final receiptsResponse = await ref
          .read(receiptProviderRepository)
          .fetchReceiptsThisYear();
      await receiptsResponse.fold<Future>((l) async {}, (r) async {
        print('Fetched ${r.length} receipts from source database');
        for (var receipt in r) {
          batch.insert(TableConstant.receiptTable, receipt.toJson());
        }
      });
      //! detailsReceipt
      final detailsReceiptsList = await ref
          .read(receiptProviderRepository)
          .fetchDetailsReceiptsThisYear();
      await detailsReceiptsList.fold<Future>((l) async {}, (r) async {
        for (var dr in r) {
          batch.insert(TableConstant.detailsReceiptTable, dr.toJsonForInsert());
        }
      });
      //! financial transactions
      final finalTransactions = await ref
          .read(receiptProviderRepository)
          .fetchFinancialTransactionsThisYear();
      await finalTransactions.fold<Future>((l) async {}, (r) async {
        for (var f in r) {
          batch.insert(TableConstant.financialTransactionTable, f.toMap());
        }
      });

      //! detailsReceipt
      final expensesList = await ref
          .read(expenseProviderRepository)
          .fetchAllExpenses();
      await expensesList.fold<Future>((l) async {}, (expenses) async {
        for (var item in expenses) {
          batch.insert(TableConstant.expensesTable, item.toMap());
        }
      });

      //! invoices (linked to receipts from this year)
      final invoices = await ref
          .read(posDbProvider)
          .database
          .rawQuery(
            '''
        SELECT * FROM ${TableConstant.invoices} 
        WHERE CAST(SUBSTR(receiptDate, 1, 4) AS integer) = ?
      ''',
            [currentYear],
          );
      for (var invoice in invoices) {
        batch.insert(TableConstant.invoices, invoice);
      }

      //! suppliers (all records)
      final suppliers = await ref
          .read(posDbProvider)
          .database
          .query(TableConstant.suppliers);
      for (var supplier in suppliers) {
        batch.insert(TableConstant.suppliers, supplier);
      }

      //! shift table (all records)
      final shifts = await ref
          .read(posDbProvider)
          .database
          .query(TableConstant.shiftTable);
      for (var shift in shifts) {
        batch.insert(TableConstant.shiftTable, shift);
      }
